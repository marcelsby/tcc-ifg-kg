...
def execute(conn: Neo4jConnection, producao_tecnica_csv: Path):
    producao_tecnica_df = pd.read_csv(producao_tecnica_csv, delimiter=";")

    transactions = producao_tecnica_df.apply(_create_producao_tecnica_transaction, axis=1)

    start = time.perf_counter()
    conn.run_transactions_batched(transactions, 500, 200)
    end = time.perf_counter()

    print(f"[ifg_produz] Produção Técnica ({producao_tecnica_df.shape[0]} linhas): {end - start}s")


def _create_producao_tecnica_transaction(row):
    create_query = _create_producao_tecnica_query(row)
    curriculo_to_producao_tecnica_rel_query = _create_curriculo_to_producao_tecnica_relationship_query(row)

    return create_query, curriculo_to_producao_tecnica_rel_query


def _create_producao_tecnica_query(row):
    properties_keys = list(row.index)

    properties_keys.remove("codigo_curriculo")

    create_query_builder = CypherCreateQueryBuilder("ProducaoTecnica")
...
def _create_curriculo_to_producao_tecnica_relationship_query(row):
